...
def should_send_summary(now, last_sent_date, hour=SUMMARY_HOUR):
    """그날 '8시 이후 첫' 점검에서 일일 요약을 보내도록 판정 — 순수함수.

    옛날엔 GitHub 무료 예약타이머가 8시에 요약을 보냈는데 1시간씩 늦잠을 잤다.
    이제 제시간에 도는 '5분 점검'이 8시 이후 그날 첫 바퀴에서 요약을 같이 보낸다.
    - now.hour 가 8 이상(한국시각 08:00 이후)이고
    - 오늘 아직 안 보냈으면(last_sent_date 가 오늘 날짜와 다르면) True.
    평소엔 8시대 첫 점검(08:00~08:05)에 나가고, 만약 8시대를 통째로 놓쳐도
    (장애 등) 9시·정오 등 그날 첫 점검에서 반드시 한 번은 보낸다('늦어도 결국 옴').
    last_sent_date 가 None이면 '아직 한 번도 안 보냄'이라 8시 이후면 보낸다.
    '마지막으로 보낸 날짜(YYYY-MM-DD)' 비교가 하루 딱 한 번을 보장한다.
    """
    return now.hour >= hour and last_sent_date != now.date().isoformat()
...
def maybe_send_daily_summary(now):
    ...
    8시대를 통째로 놓쳐도 그날 첫 점검에서 한 번은 보낸다('늦어도 결국 옴').
    """
